...
public class CargoService {
	...
	// 화물 일괄 등록
	public int insertBatchCargo(ArrayList<CargoMain> mainArr, ArrayList<CargoGoods> goodsArr, User loginUser) {
		Connection conn = JDBCTemplate.getConnection();
		int result = 0;
		...
		try {
			for (int i = 0; i < mainArr.size(); i++) {
				CargoMain main = mainArr.get(i);
				CargoGoods goods = goodsArr.get(i);
				
				ManageNoService mns = new ManageNoService();
				
                String wareCdId = mns.findZipWareCdForConn(conn,main.getReceiverZip()); //창고이동ID생성
                
                
                goods.setWarehouseMoveid(wareCdId); // 창고이동ID 
                main.setWarehouseMoveid(wareCdId); // 창고이동ID

				CargoMain existCargo = dao.searchCargoForExcelImp(conn, main);
