		...
		<main class="content d-flex">
			...
			<div class="center-div">
				...
				<div class="btn-group w-100 mb-3">
					<button id="singleRegBtn" class="btn btn-primary w-50">단건
						등록</button>
					<button id="batchRegBtn" class="btn btn-outline-primary w-50">일괄
						등록</button>
				</div>

				<!-- 화물 단건 등록 폼 -->
				<form id="cargoRegisterForm" action="/cargo/cargoRegister"
					method="POST">
					...
					<table class="table table-bordered align-middle ">
						<tbody>
							...
							<tr>
								<th>등록 회사</th>
								<td><select class="form-select" id="compCd" name="compCd"
									required>
										<c:choose>
											<c:when test="${user.userLevel eq '1'}">
												<c:forEach var="compInfo" items="${compInfoArr}">
													<option value="${compInfo.compCd}">${compInfo.compName}
													</option>
												</c:forEach>
											</c:when>
											<c:otherwise>
							...
							<tr>
								<th>송장 번호</th>
								<td><input type="text" class="form-control" id="trackingNo"
									name="trackingNo" required></td>
							</tr>
							...
							<tr>
								...
								<td><input type="text" class="form-control"
									id="receiverName" name="receiverName" required></td>
							</tr>
							...
							<tr>
								<th>수취인 주소</th>
								<td><input type="text" class="form-control"
									id="receiverAdd" name="receiverAdd" required></td>
							</tr>

							<tr>
								<th>수취인 우편번호</th>
								<td><input type="text" class="form-control"
									id="receiverZip" name="receiverZip" required></td>
							</tr>
							<tr>
								<th>수취인 전화번호</th>
								<td><input type="text" class="form-control"
									id="receiverTel" name="receiverTel" required
									pattern="\d{3}-\d{3,4}-\d{4}"></td>
							</tr>
							<tr>
								<th>판매자 이름</th>
								<td><input type="text" class="form-control" id="sellerName"
									name="sellerName" required></td>
							</tr>
							<tr>
								<th>판매자 주소</th>
								<td><input type="text" class="form-control" id="sellerAdd"
									name="sellerAdd" required></td>
							</tr>
							<tr>
								<th>판매자 전화번호</th>
								<td><input type="text" class="form-control" id="sellerTel"
									name="sellerTel" required pattern="\d{3}-\d{3,4}-\d{4}">
								</td>
							</tr>
							<tr>
								<th>상품명</th>
								<td><input type="text" class="form-control" id="goodsName"
									name="goodsName" required></td>
							</tr>
							<tr>
								<th>상품 개수</th>
							...
							<tr>
								...
								<td><input type="text" class="form-control" id="unitPrice"
									name="unitPrice" required></td>
							</tr>
							<tr>
								<th>총 중량</th>
								<td><input type="text" class="form-control" id="gw"
									name="gw" required></td>
							</tr>
							<tr>
								<th>총 중량 단위</th>
								<td><input type="text" class="form-control" id="gwt"
									name="gwt" required></td>
							</tr>
				...
				<%--일괄 등록 폼 --%>
				<form action="/cargo/cargoBatchReg" method="post"
					id="cargoBatchRegFrm" class="hidden" enctype="multipart/form-data">
					<!-- 회사 선택 영역 추가 -->
					<div class="mb-3">
						<label for="compCd" class="form-label">등록 회사</label> <select
							class="form-select" id="compCd" name="compCd" required>
							<c:choose>
								<c:when test="${user.userLevel eq '1'}">
									<c:forEach var="compInfo" items="${compInfoArr}">
					...
					<div class="mb-3">
						<label for="cargoFile" class="form-label">파일 선택</label> <input
							type="file" class="form-control" id="cargoFile" name="cargoFile">
					</div>
	...
	<script>
               ...
               */
                ...
                $('#registerBtn').on('click', function (event) {
                    ...
                    if (!trackingNo || trackingNo.length < 6 || trackingNo.length > 16) {
                        $('#trackingNo').addClass('input-error');
                        $('#trackingNo').after('<div class="error-message">송장 번호는 6~16자 이내여야 합니다.</div>');
                        isValid = false;
                    }
                    ...
                    if (!receiverAdd || receiverAdd.length < 10 || receiverAdd.length > 30 || /[^a-zA-Z0-9가-힣\s]/.test(receiverAdd)) {
                        $('#receiverAdd').addClass('input-error');
                        $('#receiverAdd').after('<div class="error-message">주소는 10~30자, 특수문자 없이 입력해야 합니다.</div>');
                        isValid = false;
                    }
                    ...
                    if (!receiverTel || !/^\d{2,3}-\d{3,4}-\d{4}$/.test(receiverTel)) {
                    ...
                    if (!sellerAdd || sellerAdd.length < 10 || sellerAdd.length > 30 || /[^a-zA-Z0-9가-힣\s]/.test(sellerAdd)) {
                        $('#sellerAdd').addClass('input-error');
                        $('#sellerAdd').after('<div class="error-message">주소는 10~30자, 특수문자 없이 입력해야 합니다.</div>');
                        isValid = false;
                    }
                    ...
                    // 총 중량 단위(GWT)
                    let gwt = $('#gwt').val();
                    if (!(gwt == 'KG' || gwt == 'LBS')) {  // 숫자가 0보다 크고, 'kg' 또는 'lbs'만 허용
